...
async function generateBundle(): Promise<void> {
	if (isDryRun) {
		...
		console.log("DRY RUN bun run gen:spa");
		return;
	}
	...
	await runCommand(["bun", "run", "gen:spa"], repoRoot);
}
...
async function resetArtifacts(): Promise<void> {
	if (isDryRun) {
		...
		console.log("DRY RUN bun run gen:spa:reset");
		return;
	}
	...
	await runCommand(["bun", "run", "gen:spa:reset"], repoRoot);
}
