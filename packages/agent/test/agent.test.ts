...
import { Agent, type AgentEvent, type AgentTool, type StreamFn, ThinkingLevel } from "@oh-my-pi/pi-agent-core";
import { type AssistantMessage, type SimpleStreamOptions, type ToolResultMessage, z } from "@oh-my-pi/pi-ai";
...
describe("Agent", () => {
	...
	it("orders Cursor native tool results by tool start positions across interspersed text", async () => {
		let agent: Agent;
		const makeToolResult = (toolCallId: string, toolName: string, text: string): ToolResultMessage => ({
			role: "toolResult",
			toolCallId,
			toolName,
		...
		const streamFn: StreamFn = (_model, _context, options?: SimpleStreamOptions) => {
			const stream = new AssistantMessageEventStream();
			const output: AssistantMessage = {
				role: "assistant",
				content: [{ type: "thinking", thinking: "inspect files first" }],
				api: "cursor-agent",
				provider: "cursor",
			...
			const pushText = (text: string) => {
				let block = output.content.find((part): part is { type: "text"; text: string } => part.type === "text");
				if (!block) {
			...
			queueMicrotask(async () => {
				...
				agent.emitExternalEvent({
					...
					toolCallId: "b",
					toolName: "grep",
					result: { content: [{ type: "text", text: "B result" }], details: {} },
					isError: false,
				});
				await options?.cursorOnToolResult?.(makeToolResult("b", "grep", "B result"));
				agent.emitExternalEvent({
					type: "tool_execution_end",
					toolCallId: "a",
					toolName: "read",
					result: { content: [{ type: "text", text: "A result" }], details: {} },
					isError: false,
				});
				await options?.cursorOnToolResult?.(makeToolResult("a", "read", "A result"));
				stream.push({
					type: "text_end",
					contentIndex: 0,
					content: "before A between A and B after B",
					partial: output,
				});
		...
		agent = new Agent({ streamFn, cursorOnToolResult: message => message });
		agent.subscribe(event => {
			if (event.type !== "message_start" && event.type !== "message_end") return;
			const message = event.message;
			if (message.role === "assistant") {
				emitted.push(
					`${event.type}:assistant:${message.content.map(part => (part.type === "text" ? part.text : "")).join("")}`,
				);
			} else if (message.role === "toolResult") {
				emitted.push(`${event.type}:tool:${message.toolName}`);
			}
		});

		await agent.prompt("go");

		const transcript = agent.state.messages.map(message => {
			if (message.role === "assistant") {
		...
		const firstAssistant = agent.state.messages.find(message => message.role === "assistant");
		if (firstAssistant?.role !== "assistant") {
			throw new Error("expected split assistant segment");
		}
		expect(firstAssistant.content).toContainEqual({ type: "thinking", thinking: "inspect files first" });
		expect(emitted).toEqual([
			"message_start:assistant:before A ",
			"message_end:assistant:before A ",
			"message_start:tool:read",
			"message_end:tool:read",
			"message_start:assistant:between A and B ",
			"message_end:assistant:between A and B ",
			"message_start:tool:grep",
			"message_end:tool:grep",
			"message_start:assistant:after B",
			"message_end:assistant:after B",
		]);
	});
